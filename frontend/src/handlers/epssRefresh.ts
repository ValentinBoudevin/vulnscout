/**
 * EPSS CVE Refresh handler — typed fetch wrappers for the EPSS refresh endpoint.
 */

import { asVulnerability } from "./vulnerabilities";
import type { Vulnerability } from "./vulnerabilities";

class EpssRefreshHandler {
    static async triggerSingleRefresh(cveId: string): Promise<Vulnerability | null> {
        const url = `${import.meta.env.VITE_API_URL}/api/vulnerabilities/${encodeURIComponent(cveId)}/epss-refresh`;
...
export default EpssRefreshHandler;
