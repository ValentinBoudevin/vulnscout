...
# ---------------------------------------------------------------------------
# import-custom-assessments: directory path
# ---------------------------------------------------------------------------

def test_import_custom_assessments_directory_variant_flag(app, tmp_path):
    """Import directory with --variant flag is rejected."""
    d = tmp_path / "vex_dir"
    d.mkdir()
    with app.app_context():
        runner = app.test_cli_runner()
    ...
    assert result.exit_code == 1
    assert "cannot use the --variant" in result.output


def test_import_custom_assessments_directory_empty(app, tmp_path):
    """Import directory with no .json files raises ValueError."""
...
def test_import_custom_assessments_directory_no_matching(app, tmp_path):
    """Import directory where no filenames match known variants."""
    d = tmp_path / "nomatch_dir"
    d.mkdir()
    # Write a valid OpenVEX file with a name that doesn't match any variant
    doc = {
        "@context": "https://openvex.dev/ns/v0.2.0",
    ...
    (d / "UnknownVariant.json").write_text(json.dumps(doc))
    with app.app_context():
        runner = app.test_cli_runner()
        result = runner.invoke(args=[
            "import-custom-assessments",
    ...
    assert result.exit_code == 1
    assert "no valid OpenVEX files" in result.output


def test_import_custom_assessments_directory_success(app, tmp_path):
    """Import directory with a matching variant file works."""
    _create_custom_assessment(app)

    # Export first to get valid OpenVEX content
    with app.app_context():
        runner = app.test_cli_runner()
        runner.invoke(args=[
            "export-custom-assessments",
    ...
    # Set up a directory with the exported file
    d = tmp_path / "import_dir"
    d.mkdir()
    import shutil
    shutil.copy(str(tmp_path / f"{_VARIANT_NAME}.json"), str(d / f"{_VARIANT_NAME}.json"))
